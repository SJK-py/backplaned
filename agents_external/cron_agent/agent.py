...
async def run_interactive_loop(
    ...
    system_prompt = (
        ...
        "ALL times are in the USER'S LOCAL TIMEZONE (shown below):\n"
        "- Cron expressions use local time.\n"
        "- start_at and end_at use local time in ISO 8601 format "
        "(e.g. 2026-04-09T09:00:00). Do NOT add Z or timezone suffix.\n\n"
        "## One-time jobs\n"
        "For tasks that should run only once:\n"
        "- Set a cron expression matching the desired time.\n"
        "- Set start_at to just before the trigger (prevents premature firing).\n"
        "- Set end_at to shortly after the trigger (e.g. 2 minutes later) so the "
        "job auto-disables after running once.\n"
        "Example: run once at 2026-04-09 9:00am → cron='0 9 9 4 *', "
        "start_at='2026-04-09T08:59:00', end_at='2026-04-09T09:02:00'\n\n"
