...
For production bare-metal deployments, set up systemd services instead of running `start.sh` interactively. Run `start.sh` once first to bootstrap `.env` files, invitation tokens, and credentials for all agents.

### Prerequisites

- Complete the [Quick Start (Bare Metal)](#quick-start-bare-metal) setup and run `start.sh` at least once
- All agents should have `data/credentials.json` after the first run
- A dedicated system user (e.g. `agora`) is recommended

### Router Service

Create `/etc/systemd/system/agora-router.service`:

```ini
[Unit]
Description=Agora Router
...
EnvironmentFile=/home/agora/agora/start.config
ExecStart=/home/agora/agora/.venv/bin/uvicorn router:app --host 0.0.0.0 --port 8000
Restart=on-failure
RestartSec=5
...
Each external agent follows the same pattern. Create one service per agent, e.g. `/etc/systemd/system/agora-web-admin.service`:

```ini
[Unit]
Description=Agora Web Admin
After=agora-router.service
Requires=agora-router.service

[Service]
Type=simple
User=agora
WorkingDirectory=/home/agora/agora/agents_external/web_admin
EnvironmentFile=/home/agora/agora/agents_external/web_admin/.env
ExecStart=/home/agora/agora/.venv/bin/uvicorn main:app --host 0.0.0.0 --port 8080
Restart=on-failure
RestartSec=5
...
Repeat for each agent, adjusting `WorkingDirectory`, `EnvironmentFile`, and port:

| Service file | Agent directory | Default port |
|---|---|---|
| `agora-channel.service` | `agents_external/channel_agent` | 8081 |
| `agora-mcp-agent.service` | `agents_external/mcp_agent` | 8082 |
| `agora-mcp-server.service` | `agents_external/mcp_server` | 8083 |
| `agora-cron.service` | `agents_external/cron_agent` | 8085 |
| `agora-kb.service` | `agents_external/kb_agent` | 8086 |
| `agora-coding.service` | `agents_external/coding_agent` | 8100 |
| `agora-reminder.service` | `agents_external/reminder_agent` | 8101 |
...
sudo systemctl enable agora-router agora-web-admin agora-channel

# Start
sudo systemctl start agora-router
sudo systemctl start agora-web-admin agora-channel  # start after router is up

# Check status
sudo systemctl status agora-router
journalctl -u agora-router -f     # tail logs
```

### Notes

- The router must be running before external agents start. Use `After=` and `Requires=` directives as shown above.
- `start.sh` handles first-time bootstrap (invitation tokens, ACL rules, config propagation). After that, systemd services can manage the processes independently.
- To add or remove agents, adjust which service files are enabled. Agents excluded via `EXCLUDE_AGENTS` in `start.config` are simply not given a systemd service.
- If you need to re-bootstrap (e.g. after adding a new agent), stop all services, run `start.sh` once, then restart the services.
